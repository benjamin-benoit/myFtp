let net = require('net');
let HOST = '127.0.0.1';
let PORT = 21000;
let messages;
let fs = require('fs');
let path = require('path');

let arguments = process.argv.splice(2);

currDir = path.join(process.cwd());

net.createServer((socket) => {
    socket.reply = (status, message, callback) => {
        if (!message)
            message = messages[status.toString()] || 'No infos';
        if (this.writable) {
            this.write(`${status.toString()} ${message.toString()}\r\n`, callback);
        }
    }

    console.log(`Server connected on --> ${socket.remoteAddress}:${socket.remotePort}`);

    socket.on('connect', () => {
        socket.reply(220);
    });

    socket.on('data', (chunk) => {
        let parts = trim(chunk.toString()).split(" "),
            command = trim(parts[0]).toUpperCase(),
            args = parts.slice(1, parts.length),
            isCallable = commands[command]

            console.log(command);

            if (!isCallable) sock.reply(502, command);
            else isCallable.apply(socket, args);
    });

    socket.dataTransfer = (selection) => {
        function finish(dataSocket) {
            return (err) => {
                if (err) dataSocket.emit('ERROR', err);
                else dataSocket.end();
            }
        }

        function execute() {
            socket.reply(150);
            selection.call(socket, this, finish.this);
        }
        dataSocket = net.createConnection(socket.dataInfo.port, socket.dataInfo.address);
        dataSocket.on('connect', execute);
    }
    socket.dataTransfer.prepare = false;
    socket.dataTransfer.queue = [];
    socket.dataTransfer.squeue = [];
}).listen(PORT, HOST);

console.log(`FTP server is listening on --> ${HOST}:${PORT}`);
console.log(`FTP server is on dir --> ${currDir}`);

commands = exports.commands = {
    "USER": (username) => {
        this.username = username
        this.reply(331);
    },
    "PASS": (password) => {
        this.reply(230)
        console.log("Log with username " + this.username + " and password " + password);
    },
    "LIST": (target) => {
        let socket = this
        socket.dataTransfer((dataSocket, finish) => {
            fs.readdir(dir, (error, file) => {
                //console.log(dataSocket);
                if (error) socket.error = true;
                else
                    file.forEach((result) => {
                        stats = fs.statSync(path.join(dir, result));
                        dataSocket.write((stats.isFile() ? "File " : "Directory ") + result + ":" + JSON.stringify(stats) + '\r\n\r\n')
                    });
                finish();
                //console.log(result);
            })
        })
        if (socket.dataTransfer.prepare)
            socket.dataTransfer.queue.shift().call(socket.dataTransfer.squeue.shift());
    },
    "CWD": (ndir) => {
        let socket = this
        fs.exists(path.join(dir, ndir), (exists) => {
            if (exists) {
                dir = path.join(dir, ndir);
                socket.reply(250, 'Directory changed to "' + ndir + '"');
            } else socket.reply(501, 'No this directory');
        });
    }, // Change the current directory of the server
    "RETR": (file) => {
        let socket = this
        socket.dataTransfer((dataSocket, finish) => {
            fs.readFile(path.join(dir, file), (err, data) => {
                console.log(data.toString());
                if (err) socket.reply(501, 'Read fail');
                else {
                    dataSocket.write(data, socket.dataEncoding);
                    dataSocket.end();
                }
            });
        })
        if (socket.dataTransfer.prepare)
            socket.dataTransfer.queue.shift().call(socket.dataTransfer.squeue.shift());
    }, // transfer a copy of the file FILE from the server to the client
    "STOR": (file) => {
        let socket = this
        socket.dataTransfer((dataSocket, finish) => {
            let value = "";
            dataSocket.on('data',(chunk) => {
                value += chunk;
            })
            dataSocket.on('end', () => {
                fs.writeFile(path.join(dir,file), value, (error) => {
                    if (error) socket.reply(501, 'Read fail');
                });
            });
        })
        if (socket.dataTransfer.prepare)
            socket.dataTransfer.queue.shift().call(socket.dataTransfer.squeue.shift());
    },
    "PWD": () => {
        this.reply(257, '"' + dir + '"')
    }, // display the name of the current directory of the server
    "HELP": () => {
        this.reply(202)
    }, // send helpful information to the client
    "QUIT": () => {
        this.reply(221)
        this.end()
    } // close the connection and stop the program
}

function trim(string) {
    return string.replace(/^\s+|\s+$/g, "")
}

messages = exports.messages = {
    "200": "Command okay.",
    "500": "Syntax error, command unrecognized.", // This may include errors such as command line too long.
    "501": "Syntax error in parameters or arguments.",
    "202": "Command not implemented, superfluous at this site.",
    "502": "Command not implemented.",
    "503": "Bad sequence of commands.",
    "504": "Command not implemented for that parameter.",
    "110": "Restart marker reply.", // In this case, the text is exact and not left to the particular implementation; it must read: MARK yyyy = mmmm Where yyyy is User-process data stream marker, and mmmm server's equivalent marker (note the spaces between markers and "=").
    "211": "System status, or system help reply.",
    "212": "Directory status.",
    "213": "File status.",
    "214": "Help message.", // On how to use the server or the meaning of a particular non-standard command. This reply is useful only to the human user.
    "215": "NodeFTP server emulator.", // NAME system type. Where NAME is an official system name from the list in the Assigned Numbers document.
    "120": "Service ready in %s minutes.",
    "220": "Service ready for new user.",
    "221": "Service closing control connection.", // Logged out if appropriate.
    "421": "Service not available, closing control connection.", // This may be a reply to any command if the service knows it must shut down.
    "125": "Data connection already open; transfer starting.",
    "225": "Data connection open; no transfer in progress.",
    "425": "Can't open data connection.",
    "226": "Closing data connection.", // Requested file action successful (for example, file transfer or file abort).
    "426": "Connection closed; transfer aborted.",
    "227": "Entering Passive Mode.", // (h1,h2,h3,h4,p1,p2).
    "230": "User logged in, proceed.",
    "530": "Not logged in.",
    "331": "User name okay, need password.",
    "332": "Need account for login.",
    "532": "Need account for storing files.",
    "150": "File status okay; about to open data connection.",
    "250": "Requested file action okay, completed.",
    "257": "\"%s\" created.",
    "350": "Requested file action pending further information.",
    "450": "Requested file action not taken.", // File unavailable (e.g., file busy).
    "550": "Requested action not taken.", // File unavailable (e.g., file not found, no access).
    "451": "Requested action aborted. Local error in processing.",
    "551": "Requested action aborted. Page type unknown.",
    "452": "Requested action not taken.", // Insufficient storage space in system.
    "552": "Requested file action aborted.", // Exceeded storage allocation (for current directory or dataset).
    "553": "Requested action not taken.", // File name not allowed.
}